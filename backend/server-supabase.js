...
app.get('/api/products', async (req, res) => {
  try {
    ...
    if (collection && collection !== 'all') {
      // Try to match both originalName and normalized_collection
    ...
    // Map products with proper image handling
...
// Get collections with Czech names
app.get('/api/collections', async (req, res) => {
  try {
    // Get collections from the collections table with Czech names
    ...
    // Create Czech translations for collection names
    const czechCollectionNames = {
      'Blue Square': 'Modrý čtverec',
      'Blue Square 2': 'Modrý čtverec 2',
      'Blue Square 3': 'Modrý čtverec 3',
      'Muse': 'Múza',
      'Muse 121': 'Múza 121',
      'Muse 129': 'Múza 129',
      'Piquadro': 'Piquadro',
      'Piquadro S2': 'Piquadro S2',
      'Coleos': 'Coleos',
      'Coleos 2': 'Coleos 2',
      'Akron': 'Akron',
      'Urban': 'Městský',
    ...
    // Transform collections to include Czech names and normalized IDs
...
app.get('/api/variants', async (req, res) => {
  try {
    ...
    // First get the base product ID from base_sku
    ...
    if (baseError || !baseProduct) {
      // Return empty array if base product not found
      return res.json([]);
    }

    // Then get variants for this base product with product details
    const { data, error } = await supabase
...
app.get('/api/stats', async (req, res) => {
  try {
    const { data: productsData, error: productsError } = await supabase
      .from('products')
      .select('id, stock, availability, created_at');
    ...
    const active = productsData.filter(p => p.stock > 0).length;
...
app.get('/api/dashboard-stats', async (req, res) => {
  try {
    ...
    // Calculate revenue from orders if available, otherwise use product inventory value
